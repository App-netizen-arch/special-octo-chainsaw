    ...
    return Shortcuts(
      shortcuts: <ShortcutActivator, Intent>{
        ...
        const SingleActivator(LogicalKeyboardKey.keyD,
            control: true, shift: true): const NewDocIntent(),
      },
      child: Actions(
        actions: <Type, Action<Intent>>{
          ...
          NewChatIntent: CallbackAction<NewChatIntent>(
              onInvoke: (_) => context.read<AppState>().newChat()),
          NewDocIntent: CallbackAction<NewDocIntent>(
              onInvoke: (_) =>
                  context.read<AppState>().setView(MainView.document)),
        },
        child: Focus(
          autofocus: true,
          child: LayoutBuilder(
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          _nav(context, Icons.chat_bubble_outline, "Ask",
              state.view == MainView.chat && state.mode != ChatMode.research,
              () {
            state.setMode(ChatMode.local);
            state.setView(MainView.chat);
            onNavigate();
          }),
          _nav(context, Icons.travel_explore, "Research",
              state.view == MainView.chat && state.mode == ChatMode.research,
              () {
          ...
          _nav(context, Icons.description_outlined, "Documents",
              state.view == MainView.document, () {
          ...
          if (state.isAuthenticated) ...[
            _nav(context, Icons.auto_awesome_outlined, "Skills",
                state.view == MainView.skills, () {
              state.setView(MainView.skills);
              onNavigate();
            }),
            _nav(context, Icons.newspaper_outlined, "Legal updates",
                state.view == MainView.legalUpdates, () {
              state.setView(MainView.legalUpdates);
              onNavigate();
            }),
          ],
          if (state.isAdmin)
            _nav(context, Icons.admin_panel_settings_outlined, "Admin",
                state.view == MainView.admin, () {
              state.setView(MainView.admin);
              onNavigate();
            }),
          _nav(context, Icons.settings_outlined, "Settings",
              state.view == MainView.settings, () {
      ...
      child: Row(
        children: [
          IconButton(
            ...
            icon: Icon(compact || !sidebarOpen ? Icons.menu : Icons.menu_open,
                size: 20),
            tooltip: compact || !sidebarOpen
                ? "Open navigation"
                : "Collapse navigation",
          ),
          ...
          if (compact)
            ...
            Flexible(
                child: ModeSelector(
                    current: state.mode, onChanged: state.setMode)),
          const Spacer(),
